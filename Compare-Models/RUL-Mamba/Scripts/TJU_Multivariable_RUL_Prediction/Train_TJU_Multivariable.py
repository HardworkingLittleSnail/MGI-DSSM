...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "Configs" / "TJU" / "Multivariable" / "Base.yaml"),
...
def load_training_dependencies():
    ...
    try:
        ...
        from Scripts.Data_Process.TJU_Data_Process import MultiVariateBatteryDataProcess
    ...
    return SimpleNamespace(
        ...
        MultiVariateBatteryDataProcess=MultiVariateBatteryDataProcess,
    )
...
def build_model_kwargs(config, enc_in, deps):
    ...
    elif model_name in {"rulmamba", "rulmambavan"}:
        kwargs = {
            "seq_len": build_args.get("seq_len", config["window"]["seq_len"]),
            "pred_len": build_args.get("pred_len", config["window"]["pred_len"]),
            "enc_in": build_args.get("enc_in", enc_in),
...
def run_repeat(
    ...
    for start_point in config["dataset"]["start_points"]:
        ...
        with open(start_log_path, "w", encoding="utf-8") as repeat_log:
            df_train, df_test, df_all = deps.MultiVariateBatteryDataProcess(
                battery_data,
