...
from NASA_Data_Process import BatteryDataProcess, BatteryDataRead, LoadBatteryCache
...
OUTPUT_ROOT = Path("Outputs/NASA/Univariable/RULMamba_Optimize")
DEFAULT_CACHE_PATH = PROJECT_ROOT / "Data" / "NASA_Data" / "NASA.npy"


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", default="RULMamba", help="Model name.")
    parser.add_argument("--seq-len", dest="seq_len", type=int, default=16, help="Input sequence length.")
    ...
    parser.add_argument("--data-dir", dest="data_dir", type=str, default=str(PROJECT_ROOT / "Data" / "NASA_Data"), help="NASA .mat dataset directory.")
    parser.add_argument("--battery-cache-path", dest="battery_cache_path", type=str, default=str(DEFAULT_CACHE_PATH), help="NASA cached dataset path.")
    parser.add_argument("--battery-list", dest="Battery_list", nargs="+", default=["B0005", "B0006", "B0007", "B0018"], help="Battery list.")
    parser.add_argument("--train-battery-list", dest="Train_Battery_list", nargs="+", default=["B0006", "B0007", "B0018"], help="Training batteries from paper Table 3.")
    parser.add_argument("--rated-capacity", dest="Rated_Capacity", type=float, default=2.0, help="Rated capacity.")
    parser.add_argument("--test-name", dest="test_name", type=str, default="B0005", help="Battery data used for test.")
    parser.add_argument("--start-points", dest="start_point_list", type=int, nargs="+", default=[50, 70, 90], help="Start cycles for prediction.")
    ...
    parser.add_argument("--n-trials", dest="n_trials", type=int, default=200, help="Number of Optuna TPE trials.")
...
def rul_value_error(y_true, y_pred, threshold):
    true_crossing = np.flatnonzero(np.asarray(y_true) <= threshold)
    pred_crossing = np.flatnonzero(np.asarray(y_pred) <= threshold)
    rul_real = int(true_crossing[0]) if true_crossing.size else len(y_true)
    rul_pred = int(pred_crossing[0]) if pred_crossing.size else len(y_pred)
...
def ensure_battery_cache(args):
    cache_path = Path(args.battery_cache_path)
    if cache_path.exists():
        return LoadBatteryCache(cache_path)
    cache_args = SimpleNamespace(
        data_dir=args.data_dir,
        battery_cache_path=str(cache_path),
        Battery_list=args.Battery_list,
        Train_Battery_list=args.Train_Battery_list,
        Rated_Capacity=args.Rated_Capacity,
        test_name=args.test_name,
        start_point_list=args.start_point_list,
    )
    return BatteryDataRead(cache_args)
...
def build_datasets(df_train, df_test, args):
    ...
    dataset_kwargs = dict(
        ...
        scalers={name: None for name in TIME_VARYING_KNOWN_REALS},
...
def load_model_checkpoint(checkpoint_path: Path, device):
    ...
    try:
        return RULMambaNetModel.load_from_checkpoint(str(checkpoint_path)).to(device=device)
    finally:
...
def run_single_start_point(repeat_index: int, start_point: int, battery_data, args):
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        ...
        df_train, df_test, df_all = BatteryDataProcess(battery_data, args.test_name, start_point, args)
...
def main():
    args = parse_args()
    set_seed(args.seed)
    battery_data = ensure_battery_cache(args)
    all_results = []
