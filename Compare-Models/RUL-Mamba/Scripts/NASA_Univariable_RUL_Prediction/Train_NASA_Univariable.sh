...
BASE_CONFIG="${PROJECT_ROOT}/Configs/NASA/Univariable/Base.yaml"
TEST_NAME="B0005"
...
Examples:
  ...
  $(basename "$0") Autoformer PatchTST --test-name B0006 -- --max-epochs 100
EOF
}
...
for MODEL in "${SELECTED_MODELS[@]}"; do
  MODEL_CONFIG="${PROJECT_ROOT}/Configs/NASA/Univariable/${MODEL}.yaml"
  echo "===== Training ${MODEL} ====="
  "${PYTHON_BIN}" "${PROJECT_ROOT}/Scripts/NASA_Univariable_RUL_Prediction/Train_NASA_Univariable.py" \
    --config "${BASE_CONFIG}" \
    --model "${MODEL}" \
