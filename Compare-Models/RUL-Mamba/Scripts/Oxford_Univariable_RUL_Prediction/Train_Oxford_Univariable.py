...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "Configs" / "Oxford" / "Univariable" / "Base.yaml"),
...
def load_training_dependencies():
    ...
    try:
        ...
        from Scripts.Data_Process.Oxford_Data_Process import BatteryDataRead, BatteryDataProcess, LoadBatteryCache
...
def to_namespace(config):
    return SimpleNamespace(
        ...
        train_cells=config["dataset"].get("train_cells", ["Cell1", "Cell3"]),
    )
...
def load_battery_data(config, deps):
    ...
    process_args = SimpleNamespace(
        data_dir=config["dataset"].get("data_dir", "Data/Oxford_Data/"),
        battery_cache_path=config["dataset"]["battery_cache_path"],
        battery_list=config["dataset"].get("battery_list", ["Cell1", "Cell3", "Cell8"]),
        train_cells=config["dataset"].get("train_cells", ["Cell1", "Cell3"]),
        Rated_Capacity=config["dataset"]["rated_capacity"],
        test_name=config["dataset"]["test_name"],
...
def run_repeat(
    ...
    for start_point in config["dataset"]["start_points"]:
        ...
        with open(start_log_path, "w", encoding="utf-8") as repeat_log:
            ...
            actuals = df_all.loc[df_all["cycle_number"] >= start_point * 100, "target"].values
