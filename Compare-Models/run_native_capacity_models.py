...
def import_dependencies(model_name: str, dataset: str):
    # PyTorch-Forecasting in the released RUL-Mamba environment is built on
    # the legacy namespace.  Mixing it with lightning.pytorch can detach the
    # closure loss after validation under PF 0.10.x.
    ...
    if model_name == "patchformer":
        ...
        from Models.Metrics import StableSMAPE as loss_class
        if dataset == "tju":
    ...
    return pl, TimeSeriesDataSet, EncoderNormalizer, NaNLabelEncoder, loss_class, Callback, EarlyStopping, ModelCheckpoint, model_class
...
def run_fold(model_name: str, dataset: str, test_battery: str, seed: int,
    ...
    pl, TimeSeriesDataSet, EncoderNormalizer, NaNLabelEncoder, loss_class, Callback, EarlyStopping, ModelCheckpoint, model_class = deps
    ...
    model = model_class.from_dataset(
        training, **config.build, learning_rate=config.learning_rate, loss=loss_class()
    )
    started = time.time()
    if checkpoint_path is None:
        # Legacy PyTorch-Forecasting prediction can leave a process-wide
        # no-grad context active in some dependency combinations.  Keep the
        # training boundary explicit so later epochs always build autograd.
