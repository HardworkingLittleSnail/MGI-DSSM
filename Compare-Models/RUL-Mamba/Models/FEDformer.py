...
from Models.Layers.FFT_Compat import compat_irfft, compat_rfft
...
class AutoCorrelation(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        q_fft = compat_rfft(queries.permute(0, 2, 3, 1).contiguous(), dim=-1)
        k_fft = compat_rfft(keys.permute(0, 2, 3, 1).contiguous(), dim=-1)
        res = q_fft * torch.conj(k_fft)
        corr = compat_irfft(res, dim=-1)
...
class FourierBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        x = compat_irfft(out_ft, n=x.size(-1))
        return (x, None)
...
class FourierCrossAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        xq_ft = compat_rfft(xq, dim=-1)
        for i, j in enumerate(self.index_q):
            if j >= xq_ft.shape[3]:
                continue
            xq_ft_[:, :, :, i] = xq_ft[:, :, :, j]
        xk_ft_ = torch.zeros(B, H, E, len(self.index_kv), device=xq.device, dtype=torch.cfloat)
        xk_ft = compat_rfft(xk, dim=-1)
        ...
        for i, j in enumerate(self.index_q):
            if i >= xqkvw.shape[3] or j >= out_ft.shape[3]:
                continue
            out_ft[:, :, :, j] = xqkvw[:, :, :, i]
        out = compat_irfft(out_ft / self.in_channels / self.out_channels, n=xq.size(-1))
        return (out, None)
...
class FourierCrossAttentionW(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        out = compat_irfft(out_ft / self.in_channels / self.out_channels, n=xq.size(-1)).permute(0, 3, 2, 1)
        return (out, None)
...
class FEDformer(nn.Module):
    ...
    def __init__(self, task_name='short_term_forecast', seq_len=96, label_len=48, pred_len=96, enc_in=7, dec_in=7, c_out=1,
                 e_layers=2, d_layers=1, n_heads=8, factor=3, d_model=16, d_ff=32, embed='timeF', freq='h',
                 dropout=0.1, moving_avg=25, version='fourier', mode_select='random', modes=32, activation='gelu'):
        """
...
class FEDFormerNetModel(BaseModel):
    def __init__(self, task_name='short_term_forecast', seq_len=24, label_len=0, pred_len=1, enc_in=7, dec_in=7, c_out=1,
                 e_layers=2, d_layers=1, n_heads=8, factor=3, d_model=16, d_ff=32, embed='timeF', freq='h',
                 dropout=0.1, moving_avg=25, version='fourier', mode_select='random', modes=32, activation='gelu', **kwargs):

        self.save_hyperparameters()
        super().__init__(**kwargs)
        self.network = FEDformer(
            task_name=task_name, seq_len=seq_len, label_len=label_len, pred_len=pred_len, enc_in=enc_in, dec_in=dec_in,
            c_out=c_out, e_layers=e_layers, d_layers=d_layers, n_heads=n_heads, factor=factor, d_model=d_model,
            d_ff=d_ff, embed=embed, freq=freq, dropout=dropout, moving_avg=moving_avg, version=version,
            mode_select=mode_select, modes=modes, activation=activation
        )
        self.label_len=label_len
...
if __name__=='__main__':
    ...
    model=FEDformer(seq_len=L, enc_in=C, dec_in=C, label_len = label_len, pred_len=1, c_out=1)
