...
BASE_CONFIG="${PROJECT_ROOT}/Configs/TJU/Multivariable/Base.yaml"
TEST_NAME="CY25_1"
...
Examples:
  ...
  $(basename "$0") Autoformer PatchTST --test-name CY25_2 -- --max-epochs 100
EOF
}
...
for MODEL in "${SELECTED_MODELS[@]}"; do
  MODEL_CONFIG="${PROJECT_ROOT}/Configs/TJU/Multivariable/${MODEL}.yaml"
  echo "===== Training ${MODEL} ====="
  "${PYTHON_BIN}" "${PROJECT_ROOT}/Scripts/TJU_Multivariable_RUL_Prediction/Train_TJU_Multivariable.py" \
    --config "${BASE_CONFIG}" \
    --model "${MODEL}" \
