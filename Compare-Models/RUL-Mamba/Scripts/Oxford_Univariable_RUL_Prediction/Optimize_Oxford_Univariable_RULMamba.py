...
from Oxford_Data_Process import BatteryDataProcess, BatteryDataRead, LoadBatteryCache
...
OUTPUT_ROOT = Path("Outputs/Oxford/Univariable/RULMamba_Optimize")
DEFAULT_CACHE_PATH = PROJECT_ROOT / "Data" / "Oxford_Data" / "Oxford.npy"


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", default="RULMamba", help="Model name.")
    parser.add_argument("--seq-len", dest="seq_len", type=int, default=10, help="Input sequence length.")
    ...
    parser.add_argument("--data-dir", dest="data_dir", type=str, default=str(PROJECT_ROOT / "Data" / "Oxford_Data"), help="Oxford data directory.")
    parser.add_argument("--battery-cache-path", dest="battery_cache_path", type=str, default=str(DEFAULT_CACHE_PATH), help="Oxford cached dataset path.")
    parser.add_argument("--battery-list", dest="battery_list", nargs="+", default=["Cell1", "Cell3", "Cell8"], help="Battery list.")
    parser.add_argument("--train-cells", dest="train_cells", nargs="+", default=["Cell1", "Cell3"], help="Train cell list.")
    parser.add_argument("--rated-capacity", dest="Rated_Capacity", type=float, default=0.74, help="Rated capacity.")
    parser.add_argument("--test-name", dest="test_name", type=str, default="Cell8", help="Battery data used for test.")
    parser.add_argument("--start-points", dest="start_point_list", type=int, nargs="+", default=[20, 30, 40], help="Start points in time_idx units.")
...
def predict_with_checkpoint(checkpoint_path: Path, test_loader, df_all, start_point: int, args):
    ...
    model = RULMambaNetModel.load_from_checkpoint(str(checkpoint_path)).to(device=device)
    predictions = model.predict(test_loader, batch_size=256)
    actuals_df = df_all.loc[df_all["cycle_number"] >= start_point * 100, ["cycle_number", "target"]]
