...
# pytorch-forecasting 0.10.3 still references aliases removed in NumPy 1.24.
# The paper environment pins NumPy 1.21.6; these aliases keep the reproduction
# runnable in a newer local environment without changing numerical behavior.
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "Configs" / "NASA" / "Univariable" / "Base.yaml"),
...
def load_training_dependencies():
    ...
    try:
        ...
        from Scripts.Data_Process.NASA_Data_Process import BatteryDataRead, BatteryDataProcess, LoadBatteryCache
    ...
    return SimpleNamespace(
        ...
        BatteryDataRead=BatteryDataRead,
        BatteryDataProcess=BatteryDataProcess,
        LoadBatteryCache=LoadBatteryCache,
    )
...
def to_namespace(config):
    ...
    return SimpleNamespace(
        ...
        Train_Battery_list=config["dataset"]["train_battery_list"],
        seq_len=config["window"]["seq_len"],
        Data_Protocol=protocol.get("data_mode", "paper"),
    )


def load_battery_data(config, deps):
    cache_path = PROJECT_ROOT / config["dataset"]["battery_cache_path"]
    if cache_path.exists():
        return deps.LoadBatteryCache(cache_path)

    process_args = SimpleNamespace(
        data_dir=config["dataset"].get("data_dir", "Data/NASA_Data/"),
        battery_cache_path=config["dataset"]["battery_cache_path"],
        Battery_list=config["dataset"].get("battery_list", ["B0005", "B0006", "B0007", "B0018"]),
        Rated_Capacity=config["dataset"]["rated_capacity"],
        test_name=config["dataset"]["test_name"],
        start_point_list=config["dataset"]["start_points"],
    )
    return deps.BatteryDataRead(process_args)
...
def load_real_data(config, torch_module):
    ...
    try:
        # PyTorch 2.6 changed the default to weights_only=True.  This trusted
        # local artifact is a NumPy capacity array generated by our data script.
        return torch_module.load(real_data_path, weights_only=False)
    except TypeError:  # PyTorch versions predating the weights_only argument
        return torch_module.load(real_data_path)


def load_model_checkpoint(model_class, checkpoint_path, torch_module, device):
    """Load a trusted local Lightning checkpoint across PyTorch 1.13-2.6+."""
...
def rul_value_error(y_test, y_predict, threshold):
    def first_eol_offset(values):
        crossing = np.flatnonzero(np.asarray(values) <= threshold)
        return int(crossing[0]) if crossing.size else len(values)

    # Offsets are measured from the prediction starting point.  For B0005,
    # cycle 125 is the first cycle <= 1.4 Ah, yielding the paper's TRUL values
    # 75, 55 and 35 at SP 50, 70 and 90 respectively.
    rul_real = first_eol_offset(y_test)
    rul_pred = first_eol_offset(y_predict)
    ae_error = abs(rul_real - rul_pred)
    re_score = ae_error / rul_real if rul_real > 0 else float(ae_error > 0)
    return rul_real, rul_pred, ae_error, re_score


def build_timeseries_dataset(
    dataframe, known_reals, unknown_reals, window_config, deps, protocol_config=None
):
    protocol_config = protocol_config or {}
    group_mode = protocol_config.get("group_id", "battery")
    if group_mode == "battery":
        group_ids = ["BatteryName"]
    elif group_mode == "constant":
        group_ids = ["constant"]
    else:
        raise ValueError(f"Unsupported NASA group-id protocol: {group_mode}")
    ...
    scaler_mode = protocol_config.get("input_scaler", "none")
    if scaler_mode == "none":
        dataset_kwargs["scalers"] = {name: None for name in known_reals}
    elif scaler_mode != "default":
        raise ValueError(f"Unsupported NASA input-scaler protocol: {scaler_mode}")
...
def build_dataloaders(df_train, df_test, config, deps):
    ...
    protocol_config = config.get("reproduction_protocol", {})
    ...
    testing = build_timeseries_dataset(
        df_test, known_reals, unknown_reals, config["window"], deps, protocol_config
    )
...
def build_model_kwargs(config, enc_in, deps):
    ...
    if model_name in {"autoformer", "fedformer"}:
        ...
        kwargs["patch_len"] = build_args.get("patch_len", 4)
        kwargs["stride"] = build_args.get("stride", 2)
        kwargs["c_out"] = build_args.get("c_out", 1)
        kwargs["e_layers"] = build_args.get("e_layers", 2)
        kwargs["n_heads"] = build_args.get("n_heads", 4)
        ...
        kwargs = {
            ...
            "pred_len": build_args.get("pred_len", config["window"]["pred_len"]),
            "enc_in": build_args.get("enc_in", enc_in),
            "k": build_args.get("k", 2),
            "patch_size_list": build_args.get("patch_size_list", [8, 4, 2, 1]),
            "learning_rate": config["train"]["learning_rate"],
            "loss": deps.SMAPE(),
        }
        ...
        kwargs = {
            ...
            "enc_in": build_args.get("enc_in", enc_in),
            "c_out": build_args.get("c_out", 1),
            "d_model": build_args.get("d_model", 48),
            "n_dec_layer": build_args.get("n_dec_layer", 1),
            "dropout": build_args.get("dropout", 0.1),
            "expand": build_args.get("expand", 2),
            "learning_rate": config["train"]["learning_rate"],
    ...
    for start_point in config["dataset"]["start_points"]:
        ...
        with open(start_log_path, "w", encoding="utf-8") as repeat_log:
            ...
            best_model = load_model_checkpoint(
                model_class, best_model_path, deps.torch, device
            )
...
def main():
    ...
    battery_data = load_battery_data(config, deps)
    ...
    with open(log_path, "w", encoding="utf-8") as aggregate_log:
        ...
        write_log(
            f"reproduction_protocol={config.get('reproduction_protocol', {})}",
            aggregate_log,
        )
